      ...
      <div className="animate-fade-in">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="text-xs">{town.numZones} Districts</Badge>
          </div>
        </div>
                ...
                {town.zones.map((z, i) => (
                  <>
                    <TableRow key={z.code} className={`cursor-pointer ${i % 2 === 0 ? "bg-card" : "bg-secondary/20"} hover:bg-secondary/40`} onClick={() => setExpandedZone(expandedZone === z.code ? null : z.code)}>
                      ...
                      <TableCell><Badge variant="outline" className="text-[10px] font-mono">{z.variance === "c" ? '"c" bulk' : '"d" use'}</Badge></TableCell>
            ...
            <Card key={z.code}>
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <Badge variant="outline" className="text-[10px] font-mono ml-auto">{z.variance === "c" ? '"c"' : '"d"'}</Badge>
                </div>
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs">
                  ...
                  <div><span className="text-muted-foreground">Setbacks:</span> <span className="font-medium font-mono">{z.setbacks.front}/{z.setbacks.side}/{z.setbacks.rear}</span></div>
                  <div><span className="text-muted-foreground">Coverage:</span> <span className="font-medium">{z.maxCoverage}</span></div>
                </div>
                ...
                {expandedZone === z.code && (
                  <div className="mt-3 pt-3 border-t space-y-3">
                    <div><p className="text-xs font-semibold text-success mb-1">Permitted</p>{z.permitted.map((u) => <p key={u} className="text-xs text-muted-foreground">• {u}</p>)}</div>
                    <div><p className="text-xs font-semibold text-warning mb-1">Conditional</p>{z.conditional.map((u) => <p key={u} className="text-xs text-muted-foreground">• {u}</p>)}</div>
                    <div><p className="text-xs font-semibold text-destructive mb-1">Prohibited</p>{z.prohibited.map((u) => <p key={u} className="text-xs text-muted-foreground">• {u}</p>)}</div>
                  </div>
                )}
              </CardContent>
            </Card>
